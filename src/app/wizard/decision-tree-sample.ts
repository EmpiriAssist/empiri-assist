...
.directive('decisionTree', ['$compile', '$timeout', function($compile, $timeout) {
    ...
    return {
        ...
        templateUrl: 'decision-tree-sample.html',
            ...
            scope.$watch('decision.selected', function (node) {
                ...
                if (node) {
                    ...
                    if (node.result) {
                        ...
                        var el = angular.element('<decision-tree-sample node="decision.selected" on-result="onResult({result: result})" on-clear="onClear()"></decision-tree-sample>');
                        $compile(el)(scope);
...
.directive('modalBody', ['$compile', '$templateRequest', function ($compile, $templateRequest) {
    ...
    return {
        ...
        link: function (scope, element, attributes) {
            ...
            var resultContainer = element.find('div.decision-tree-sample-result');
    ...
    return {
        ...
        openWizard: function (config) {
            ...
            return $uibModal.open({
                animation: false,
                templateUrl: 'decision-tree-sample-wizard-modal.html',
                resolve: {
